...
const LineChart = ({ name, ohlcData, error, isError, }) => {
    ...
    const drawChart = (data, fetchedDates) => {
        const canvas = canvasRef.current;
        if (!canvas)
            return;
        const ctx = canvas.getContext("2d");
    ...
    return (_jsx(motion.div, { className: "max-w-4xl p-4 mx-auto", initial: { opacity: 0, y: 40 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.4 }, children: _jsxs(Card, { className: "border shadow-xl rounded-xl", children: [_jsx(CardHeader, { className: "flex flex-row items-center justify-between", children: _jsx(CardTitle, { className: "text-lg sm:text-2xl", children: _jsxs(HoverCard, { children: [_jsxs(HoverCardTrigger, { className: "flex items-center gap-2", children: [_jsxs("span", { children: [name.charAt(0).toUpperCase() + name.slice(1), " - Last 7 Days"] }), isError ? (_jsx(AlertCircle, { className: "w-4 h-4 text-red-500" })) : (_jsx(Info, { className: "w-4 h-4 text-muted-foreground" }))] }), _jsx(HoverCardContent, { className: "text-sm", children: isError
                                        ? "Error loading line chart data. Please try again."
                                        : "Line chart of the closing prices for the last 7 days." })] }) }) }), _jsx(CardContent, { children: _jsx("canvas", { ref: canvasRef, width: 800, height: 400, onMouseMove: handleMouseMove, className: "w-full max-w-full border rounded-md" }) })] }) }));
};
